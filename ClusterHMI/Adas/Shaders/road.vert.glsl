...
void main()
{
    ...
    vec4 positionWorld = localToWorldMatrix * vec4(pos.xyz, 1.0);
    viewDirection = positionWorld.xyz - kzCameraPosition;
    ...
    gl_Position = kzProjectionCameraWorldMatrix * localToSkinMatrix * vec4(pos.xyz, 1.0);
    ...
    vec3 V = normalize(positionWorld.xyz * kzCameraPosition.w - kzCameraPosition.xyz);
    normal = normalize((kzMorphTarget0Normal * kzMorphWeights[0]) +
    ...
    vec4 pos = vec4(kzCameraPosition.xyz, 1.0);
    pos *= rotate2d(90.0);
    ...
    vec4 positionWorld = kzWorldMatrix * vec4(pos.xyz, 1.0);
    viewDirection = positionWorld.xyz - kzCameraPosition;
    vec3 V = normalize(viewDirection);
