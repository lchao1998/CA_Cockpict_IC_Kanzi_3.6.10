...
void main()
{
...
#if KANZI_SHADER_USE_BASECOLOR_TEXTURE
    lowp vec4 baseColor = texture2D(Texture, vTexCoord + TextureOffset).rgba * texture2D(TextureMask, vTexCoord+TextureOffset).a;
#else
    ...
    gl_FragColor = vec4(1.0,0.0,0.0,1.0) * BlendIntensity;
}